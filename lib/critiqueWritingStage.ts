...
- Voice B phase3.teacherNextSteps (required for all eight categories): For each category, phase3.teacherNextSteps is the readable studio guidance derived from categories[].plan for THAT criterion on THIS painting only.
  ...
  - **Clarity and grammar (mandatory):** Write **two or three short sentences** (or one longer sentence only if it stays under ~45 words). Do not paste anchor.areaSummary as a headline and then repeat it inside the same step. Do not stack two independent observations before the imperative; lead with **one** concrete visible read, then the move, then the outcome. After a lead like "In the [passage], …", the next word must be **lowercase** unless it is a proper noun. Do not repeat the same idea twice (e.g. "seclusion" / "tranquility" in both the lead and the "so …" clause unless the second adds a new visual read).
...
- Shared anchor rules (required for every criterion):
  ...
  - **Region placement (mandatory):** Infer the box **only** from what is visible in THIS photograph. The rectangle must cover the **same concrete forms and relationships** named in areaSummary and evidencePointer—do not center on unrelated background (large empty sky, bare canvas, or distant filler) unless the anchor text is explicitly about that area alone. Prefer a **tight** box: most pixels inside it should belong to the passage being discussed. When the anchor describes a **relationship between two visible passages** (one shape against another, one mass blending into another, one edge meeting another), the box must include **both** sides of that relationship; if they are spatially separated, span from one to the other so their **junction** lies inside the rectangle rather than above or beside it.
  - **Vertical band sanity:** If the anchor names **foreground**, **bottom**, **near the viewer**, **along the path**, **stairs**, **bridge**, **railing**, **walkway**, or similar, **y + height must extend far enough toward y≈1** that the box actually covers that lower passage—do not place the box in the upper half or centered on distant trees when the text is about the foreground or a structure entering from the bottom. If the anchor names **sky**, **upper canopy only**, or **distant background**, the box must sit in the **upper** portion; do not swallow the whole canvas. For **diagonal structures** (bridge, path, railing), align the box along that diagonal (elongated width or height) so the named structure occupies **most** of the box interior, not a generic centered square.
