...
function expertAssessment(critique: Awaited<ReturnType<typeof runOpenAICritique>>): string[] {
  ...
  const genericMainIssue = simple
    ? /(clearer focal|stronger focal|enhance depth|more depth|more contrast|spatial definition|guide the viewer)/i.test(
        simple.mainIssue
      )
    : false;
  const genericNextSteps = simple
    ? simple.nextSteps.some((step) =>
        /(increase contrast|enhance definition|refine edges|create a stronger focal point|improve spatial clarity)/i.test(
          step
        )
      )
    : false;
