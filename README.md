...
**AI preview (image edit):** After a critique, you can run **Generate AI edit** on **each criterion** that has an anchor (and switch between multiple previews in one session). The default preview target follows the **first suggested studio change** from synthesis (each change names its `previewCriterion`); if that list is missing—e.g. some older saved JSON—the client falls back to the **first criterion** in the response. The server calls OpenAI **image edit** (`gpt-image-2.5-sunburst` by default), uses **sharp** to read the upload’s pixel size, pick the closest allowed API canvas (`1024×1024`, `1536×1024`, or `1024×1536`), then **resizes the model output back to the same width × height** as your upload so the compare slider aligns exactly. Optional env: `OPENAI_IMAGE_EDIT_MODEL` (e.g. `gpt-image-2` or `gpt-image-1.5`), `OPENAI_IMAGE_EDIT_QUALITY` (defaults to **medium** for speed; `low` / `high` / `xhigh` / `max` on Sunburst), `OPENAI_IMAGE_EDIT_CANDIDATES` (defaults to **1** so previews return quickly; set `2`–`4` to generate multiple candidates in parallel and return the one that best matches the upload—gpt-image-2 / 2.5 only accept `n: 1` per request, so multi-candidate mode fans out across parallel requests). `input_fidelity=high` is only sent for `gpt-image-1.*` models; gpt-image-2 / 2.5 apply high-fidelity reference handling automatically and reject that parameter.
...
2. **Vercel env — set these Key / Value pairs** (Production + Preview), then Redeploy.  
   Requires **Vercel Pro** (300s). Hobby still kills critiques at ~60s.

   | Key | Value | Required? |
   | --- | --- | --- |
   | `OPENAI_API_KEY` | `sk-…` | Yes |
   | `OPENAI_MODEL` | `gpt-6-astra` | Yes — drives looking, 8 writers, and synthesis |
   | `OPENAI_IMAGE_EDIT_MODEL` | `gpt-image-2.5-sunburst` | Yes for AI previews |
   | `OPENAI_IMAGE_EDIT_QUALITY` | `high` | Yes for AI previews |

   **Do not set** (delete if present): `OPENAI_MODEL_WRITE`, `OPENAI_MODEL_EVIDENCE`, `OPENAI_MODEL_VALIDATE`, `OPENAI_CRITIQUE_MODEL`, `OPENAI_REASONING_EFFORT`, `OPENAI_CLARITY_PASS`.
