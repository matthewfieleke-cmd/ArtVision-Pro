...
2. **Add the secret in Vercel**  
   ...
   - Optional: `OPENAI_MODEL` (default `gpt-5.4` for shared chat/classify fallback; `gpt-6-astra` is supported — the server sends `reasoning_effort` and omits `temperature`).
   ...
   - Optional: `OPENAI_REASONING_EFFORT` (`low` | `medium` | `high` | `xhigh` | `max`) — global **ceiling** for the `reasoning_effort` sent to gpt-5 / gpt-6 / o-series reasoning models. Each stage picks a sensible default (low for classify, medium for per-criterion writing, high for the vision and synthesis passes); this env var can only pull a stage down, never raise it. Ignored for non-reasoning chat models like gpt-4o or `gpt-5-chat-latest`, which use `temperature` instead.
