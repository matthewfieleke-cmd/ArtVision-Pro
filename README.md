...
**AI preview (image edit):** After a critique, you can run **Generate AI edit** on **each criterion** that has an anchor (and switch between multiple previews in one session). The default preview target follows the **first suggested studio change** from synthesis (each change names its `previewCriterion`); if that list is missing—e.g. some older saved JSON—the client falls back to the **first criterion** in the response. The server calls OpenAI **image edit** (`gpt-image-2` by default), uses **sharp** to read the upload’s pixel size, pick the closest allowed API canvas (`1024×1024`, `1536×1024`, or `1024×1536`), then **resizes the model output back to the same width × height** as your upload so the compare slider aligns exactly. Optional env: `OPENAI_IMAGE_EDIT_MODEL` (override to `gpt-image-1.5` if you want the older model), `OPENAI_IMAGE_EDIT_QUALITY` (defaults to **high** for sharper previews; set `medium` or `low` to reduce cost), `OPENAI_IMAGE_EDIT_CANDIDATES` (defaults to **3**; multi-candidate previews are ranked against the upload and the best-matching one is returned—gpt-image-2 only accepts `n: 1` per request, so the server fans out across parallel requests when the candidate count exceeds 1). `input_fidelity=high` is only sent for `gpt-image-1.*` models; gpt-image-2 applies high-fidelity reference handling automatically and rejects that parameter.
